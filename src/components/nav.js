...
class Nav extends Component{
    ...
    renderButton(){
        if(this.props.auth){
            return <button onClick={()=>this.props.changeAuth(false)} className='btn btn-outline-dark'>Sign Out</button>
        }
    ...
    render(){
        ...
        return (
            <nav className='navbar bg-primary mb-4'>
                ...
                <Link style={linkStyle} className='nav-link' to='/operatives-list'>Operative List</Link>
                {this.renderButton()}
            </nav>
        )
    }
